#!/usr/bin/env python3
"""Generate FieldOps project tracker HTML files."""
    ...
    },
    {
        ...
        "tasks": [
            ...
            },
            {
                ...
                "description": "Ensure admin@syntaxure.dev StaffMember has OWNER role and is treated as super admin in guards.",
...
def total_tasks():
    return sum(len(p["tasks"]) for p in phases)


def phase_progress_key(phase_id):
    return f"phase-{phase_id}-progress"


def task_key(phase_id, task_idx):
    return f"task-{phase_id}-{task_idx}"
...
/* Mobile list view simplification */
      ...
      <article class="phase-card" data-phase-id="{p['id']}">
        ...
        <div class="phase-meta">
          <div class="phase-progress">
            ...
            <span class="phase-count">0/{task_count}</span>
          </div>
...
""")
    ...
    manual_items = "\n".join(
        f"""      <div class="manual-card">
        <h4>{escape(m['title'])}</h4>
...
def render_phase(phase, prev_phase=None, next_phase=None):
    tasks_html = []
    for idx, t in enumerate(phase["tasks"]):
        is_manual = t.get("manual", False)
        manual_badge = '<span class="badge badge-manual">Manual</span>' if is_manual else ''
        db_badge = '<span class="badge badge-db">DB</span>' if 'db/prisma' in ' '.join(t.get('files', [])) else ''
        files_html = '\n'.join(f'              <li><code>{escape(f)}</code></li>' for f in t.get('files', []))
        tasks_html.append(f"""
      <div class="task-item" data-task-key="task-{phase['id']}-{idx}">
        <div class="task-row">
          ...
          <div class="task-badges">{manual_badge}{db_badge}</div>
        </div>
        <div class="task-body">
          <p>{escape(t['description'])}</p>
          <p><strong>Commit:</strong> <code>{escape(t['commit'])}</code></p>
          <ul>
{files_html}
          </ul>
        </div>
...
""")
    ...
    nav_links = []
    if prev_phase:
        nav_links.append(f'<a class="btn" href="phase-{prev_phase["id"]}.html">← P{prev_phase["id"]}</a>')
    nav_links.append('<a class="btn btn-primary" href="index.html">All Phases</a>')
    if next_phase:
        nav_links.append(f'<a class="btn" href="phase-{next_phase["id"]}.html">P{next_phase["id"]} →</a>')
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>P{phase['id']} — {escape(phase['title'])} | FieldOps Tracker</title>
  <link rel="stylesheet" href="styles.css" />
</head>
<body>
  <header class="sticky-header">
    <div class="container inner">
      <h1>P{phase['id']} — {escape(phase['title'])}</h1>
      <div class="progress-wrap">
        <div class="progress-bar"><div class="progress-fill" id="global-progress-fill"></div></div>
        <span class="progress-text" id="global-progress-text">0% complete</span>
      </div>
...
def main():
    ...
    (OUTPUT_DIR / "styles.css").write_text(COMMON_CSS, encoding="utf-8")
    (OUTPUT_DIR / "script.js").write_text(COMMON_JS, encoding="utf-8")
    ...
    for i, phase in enumerate(phases):
        ...
        (OUTPUT_DIR / f"phase-{phase['id']}.html").write_text(
            render_phase(phase, prev_phase, next_phase), encoding="utf-8"
        )

    print(f"Generated {len(phases) + 1} HTML files, styles.css, and script.js in {OUTPUT_DIR}")
